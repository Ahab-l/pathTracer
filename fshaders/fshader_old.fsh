...
void main(){
    Ray ray;
    vec2 coordsTile = mix(tileoffset, tileoffset + invNumTiles, Textcoords);
    coordsTile = coordsTile * 2.0 - 1.0;
    ...
    for(int i=0;i<spp;i++){
        ...
        vec4 dir       = cameraRotate*vec4(coordsTile.xy+AA,-1.5,0.0);//(pix.xy, 2) - ray.startPoint;
    ...
    vec3 lastColor = texture2D(lastFrame, coordsTile.xy*0.5+0.5).rgb;
